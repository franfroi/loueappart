...
if (isset($_FILES['foto3']) AND $_FILES['foto3']['error'] == 0)
{
    // Testons si le fichier n'est pas trop gros
    if ($_FILES['foto3']['size'] <= 1000000)
    {
        // Testons si l'extension est autorisée
        $infosfichier = pathinfo($_FILES['foto3']['name']);
        ...
        if (in_array($extension_upload, $extensions_autorisees))
        {
            ...
            $newName = hash('sha1',$_FILES['foto3']['name']).'.'.$extension_upload;
            
            move_uploaded_file($_FILES['foto3']['tmp_name'], 'Images/' . basename($newName));
